                                ...
                                {cartItems.map(item => (
                                    <div key={item._id} className="flex gap-3">
                                        <div className="h-12 w-12 rounded-lg bg-gray-50 border border-gray-100 p-1 flex-shrink-0">
                                            {item.image ? (
                                                <img src={item.image.startsWith('http') ? item.image : `${import.meta.env.VITE_API_URL ? import.meta.env.VITE_API_URL.replace("/api", "") : "http://localhost:8080"}${item.image}`} alt="" className="h-full w-full object-contain mix-blend-multiply" />
                                            ) : (
